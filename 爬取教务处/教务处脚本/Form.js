...
CKForm.prototype.exp = function(url, frmname) {	// 查询表单
	...
	if(!this.form.target)  {
		...
		for(var target in this.tableId) {
			if (url && frmname){ 
				window.frames(frmname).location.href = url ;
			} else {
				this.form.action = this.expsrc + "?tableId=" + this.tableId[target];	
		...
		if (url && frmname){ 
			...
			this.form.action = this.expsrc + "?tableId=" + this.tableId[this.form.target];	
			//this.lastOptType = this.form.elements["hidOption"].value = "EXP";
			//IKLoader_StartLoading("",this);
			this.form.submit();
		}
